...
public class PresetHandler {
    ...
    private VBox createPresetPage(int pageIndex, FlowPane base) {
        VBox box = new VBox(5);
        for (int index = pageIndex; index < pageIndex + 1; index++) {
            TextArea text = new TextArea(presets[index]);
            text.setWrapText(true);
            currentPreset = Character.toUpperCase(presets[index].charAt(0)) + presets[index].substring(1);
            Label label = new Label(currentPreset);
            HBox nameAndOpen = new HBox(5);
            nameAndOpen.getChildren().addAll(label);
            File fileOne = new File("Presets/"+presets[index]+".png");
            if(fileOne.exists() && !fileOne.isDirectory()) {
                Image myPreset = new Image("file:Presets/"+presets[index]+".png");
                ImageView myPresetView = new ImageView();
                myPresetView.setImage(myPreset);
                box.getChildren().add(myPresetView);
            } else {
                File file = new File("Presets/nopreview.png");
